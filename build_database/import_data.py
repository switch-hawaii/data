...
def mini_timeseries():
    ...
    create_mini_timeseries(pd.DataFrame({
        'time_sample': 'tiny',
        'period': [2025, 2045],
    }))
...
def create_mini_timeseries(study_periods):
    ...
    study_dates['timeseries'] = study_dates['period'].mod(100)*100 + study_dates['month_of_year']

    one_day = pd.DataFrame({
        'hour_of_day': np.arange(0, 24, tp_duration),
    })
    study_hours = (
        study_dates.loc[:, ['time_sample', 'timeseries', 'date']].assign(dummy=1)
    ...
    study_hours['timepoint'] = study_hours['timeseries'] * 100 + study_hours['hour_of_day']
...
def annual_slice_timeseries():
    ...
    study_hours = study_dates[
        ['period', 'base_year', 'time_sample', 'timeseries', 'month_of_year', 'day_of_month']
    ].merge(
        year_hours[['month_of_year', 'day_of_month', 'hour_of_day']],
        on=['month_of_year', 'day_of_month']
    )
...
def make_records_from_sample_dates(
    ...
    study_dates['timeseries'] = (study_dates['period']%100)*1000000 + dt.strftime('%y%m%d').astype(int)
...
def read_simple_ev_adoption_file(file_name):
    ...
    ev_adoption_curves = get_table_from_xlsx(
        data_dir("EV Adoption", file_name),
        named_range='ev_data'
    )
    ...
    final_cols = [
        "ICE miles per gallon", "EV miles per kWh",
...
def ev_hourly_charge_profiles():
    ...
    das_profile = pd.read_csv(
        data_dir('EV Adoption', 'ev_hourly_charge_profile.tsv'),
        sep='\t'
    )
...
def fuel_costs():
    ...
    files = [
        (
            'EIA AEO {} {}.xlsx'.format(year, name),
            'AEO_{}_{}'.format(year, scenario)
        )
    ...
    for col, tag in [('high', 'High_Oil_Prices'), ('mid', 'Reference'), ('low', 'Low_Oil_Prices')]:
        ...
        import_monthly_fuel_costs(
            ...
            monthly_file=eia_dir('Roberts monthly crude_oil_price_projections_2020_2045_2019dollar.csv'),
            monthly_column=col,
...
def import_monthly_fuel_costs(
    ...
    crude_mmbtu_per_bbl = get_named_cell_from_xlsx(annual_file, named_range='brent_crude_mmbtu_per_bbl')
    monthly.loc[:, 'brent_cost_per_mmbtu'] = monthly.loc[:, 'brent_cost_per_bbl'] / crude_mmbtu_per_bbl
    ...
    if any(annual['load_zone'] != 'Oahu'):
        raise NotImplementedError('Monthly fuel cost code must be updated for non-Oahu load zones.')
    ...
    all_monthly.update(  # .update is in-place only
        monthly.stack().to_frame(name='price_mmbtu')
        .reset_index().set_index(['fuel', 'year', 'month'])
    )
...
def import_eia_fuel_costs(file, fuel_scenario):
    ...
    print("Added EIA-derived forecast (fuel_scenario={}) to fuel_costs table.".format(fuel_scenario))
...
def import_hedged_fuel_costs(file, tag='hedged'):
    ...
    prices = (
        prices.T.set_index(['fuel_type', 'tier']).T
        .replace({'#DIV/0!': float('nan'), 0: float('nan')})  # remove missing values
        .astype(float)
    )
    ...
    prices = prices.set_index(['fuel_scenario', 'year', 'fuel_type', 'tier']).sort_index()
    # rename fuel_type
    ...
    print("Added hedged prices (fuel_scenario = {}) to fuel_costs table.".format(
...
def import_psip_fuel_costs(file):
    ...
    print("Added PSIP prices (fuel_scenario = {}) to fuel_costs table.".format(fuel_scenario))
...
def onshore_wind():
    ...
    wind_proj = wind_proj.rename({
        'max_capacity': 'gen_capacity_limit_mw'
    }, axis=1)

    # De-rate Kawailoa and Kahuku to match historical capacity factor
    # as reported in EIA Form 923 (2012-2018)
    for s, cf in [('OnWind_Kawailoa', 0.183), ('OnWind_Kahuku', .241)]:
    ...
    print("Copying definitions for onshore wind farms into projects and variable_capacity_factors tables (abt. 2 mins)")
    ...
    wind_proj.loc[:, [
        ...
        'gen_capacity_limit_mw', 'latitude', 'longitude',
...
def new_generator_info():
    ...
    def get_technology_cost_series(region):
        ...
        df.columns.names = (
            ...
            c.replace('tech_scen_id', 'tech_scenario') for c in df.iloc[:,0].name
        )
    ...
    print("creating gen_build_costs table with data for future generation technologies.")
    ...
    print("creating gen_part_load_fuel table with data for future generation technologies.")
    ...
    # inserting new ones
    execute(
        'DELETE FROM projects WHERE technology NOT IN %s',
...
def existing_generator_info():
    ...
    gen_info = gen_info.rename(columns={
        'fuel': 'gen_energy_source',
...
def calibrated_sls(sls):
    ...
    def loads_exp():
        ...
        print("Not saving load_scenario {} because loads_exp has not been completely written.")
